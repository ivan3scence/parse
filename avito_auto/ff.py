...
search = 'ford+focus+ii-restayling-20082011_4596'     # Строка поиска на сайте и ниже параметры выбора города, радиуса разброса цены и т.п.
categoryId = 9
locationId = 638480         # Новосибирск
searchRadius = 400
priceMin = 0
priceMax = 10000000000
sort = 'desc'
withImagesOnly = 'false'     # Только с фото
...
cikle = 0
cikle_end = 2               # Переменная для перебора страниц с объявлениями
...
for i in items: # Теперь идем по ябъявлениям:
    ...
    if not 'error' in more_data_2:
        ...
        print('=======================================================\n')
        with open("ff.csv",'a') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow((url_more_data_2, more_data_2['title'], phone_number))
            # for sheet in self.dick.keys():
            #     writer.writerow((sheet, self.dick[sheet]))